...
def evaluate(sess,args,model,data):
    ...
    #(eval_info, class_info), eval_loss = model.evaluate(sess,data.test_Ids,metric,args.batch_size)
...
def main():
    ...
    args.output_dir = config.output_dir / Path('{}'.format(str(config.dataset)+'-on-lstm'))
