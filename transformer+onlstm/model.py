...
from relative_transformer_module import relative_multi_head_attention,feedforward,local_multi_head_attention
import optimization
import math
from common import batchify_with_label
from nn_utils import shape,get_local_bias
...
class NERModel(object):
    def __init__(self,config,data):
        ...
        #on-lstm
        inputs = self.onLSTM_layer(word_input_cat,self.config['attention_size'],self.word_seq_lengths,1,16)
            ...
            with tf.variable_scope("num_blocks_{}".format(block_id)):
                if block_id==0:
                  attn_outs = local_multi_head_attention(
                      attn_outs, mask, attention_size, num_heads, attention_keep_prob,reuse=False,local_bias=local_bias)
                else:
    ...
    def onLSTM_layer(self,lstm_inputs,lstm_dim,lengths,num_layers,chunk_size,keep_prob=1.):
      with tf.variable_scope("on_LSTM"):
        for layer in range(num_layers):
          with tf.variable_scope("layer_{}".format(layer)):
            cell_fw = ON_LSTM(lstm_dim,chunk_size)
            outputs,_ = tf.nn.dynamic_rnn(cell=cell_fw,inputs=lstm_inputs,sequence_length=lengths,time_major=False,dtype=tf.float32)
            lstm_inputs=outputs
        return lstm_inputs
