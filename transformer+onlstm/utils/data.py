...
class Data:
    ...
    def build_alphabet(self, input_file):
        ...
        for idx in range(len(in_lines)):
            ...
            chars,bigrams,labels=get_word_label_list(line)
            for char in chars:
                self.word_alphabet.add(char)
            for label in labels:
                self.label_alphabet.add(label)
            for bigram in bigrams:
                self.biword_alphabet.add(bigram)
                self.biword_count[bigram]=self.biword_count.get(bigram,0) + 1
    ...
    def build_gaz_alphabet(self, input_file, count=False):
        ...
        for line in in_lines:
            ...
            for idx in range(w_length):
                matched_entity = self.gaz.enumerateMatchList(chars[idx:])
    ...
    def generate_instance_with_gaz(self, input_file, name):
        self.fix_alphabet()
        if name == "train":
            self.train_texts, self.train_Ids = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet,self.biword_alphabet, self.biword_count,  self.gaz_alphabet, self.gaz_count, self.gaz_split,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "dev":
            self.dev_texts, self.dev_Ids = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet,self.biword_alphabet, self.biword_count, self.gaz_alphabet, self.gaz_count, self.gaz_split,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "test":
            self.test_texts, self.test_Ids = read_instance_with_gaz(input_file, self.gaz, self.word_alphabet,self.biword_alphabet, self.biword_count,  self.gaz_alphabet, self.gaz_count, self.gaz_split,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        elif name == "raw":
            self.raw_texts, self.raw_Ids = read_instance_with_gaz(input_file, self.gaz,self.word_alphabet,self.biword_alphabet, self.biword_count,  self.gaz_alphabet, self.gaz_count, self.gaz_split,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        else:
